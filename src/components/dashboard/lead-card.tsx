      ...
      <div>
        <div className="flex justify-between items-start mb-3">
          <div>
            ...
            <p className="text-zinc-400 text-xs flex items-center gap-1">
              <MapPin size={12} />
              {lead.city || "Unknown Area"} {lead.zip_code && `(${lead.zip_code})`}
            </p>
          </div>
        </div>
        ...
        <div className="flex gap-4 mb-5 text-xs">
          <div className="flex items-center gap-1">
            <Star
              size={14}
              className={lead.rating && lead.rating < 4 ? "text-red-400" : "text-[#ffe600]"}
            />
            <span
              className={`font-bold ${lead.rating && lead.rating < 4 ? "text-red-400" : "text-white"}`}
            >
              {lead.rating || "N/A"}
            </span>
      ...
      <div className="mt-auto border-t border-zinc-800 pt-4">
        {lead.is_unlocked ? (
          ...
          <div className="space-y-2.5 animate-in fade-in duration-500">
            ...
            {/* Fallback if somehow they unlocked a lead with zero contact info */}
            {!lead.phone && !lead.email && (
              <div className="text-zinc-500 text-xs italic">No contact data found.</div>
            )}
          </div>
          
        ) : (
          
          // --- LOCKED STATE: Honest Blurring & Paywall ---
          <div className="relative rounded-xl overflow-hidden border border-zinc-800/50 bg-zinc-900/20 p-3 min-h-[64px]">
            
            {/* Blurred Fake Data (Only shows if the real data exists in DB) */}
            <div className="space-y-2.5 blur-[5px] opacity-40 select-none pointer-events-none">
              ...
              {!lead.phone && !lead.email && (
                <div className="text-sm text-zinc-400">No contact info available</div>
              )}
            </div>
            ...
            <div className="absolute inset-0 flex flex-col items-center justify-center bg-[#0b0a0b]/40 backdrop-blur-[2px]">
              {lead.phone || lead.email ? (
                ...
                <span className="text-xs text-zinc-500 font-medium">No Data to Unlock</span>
              )}
            </div>
          </div>
        )}
